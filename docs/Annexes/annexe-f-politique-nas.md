...
La sync **respecte le quota** de l'organisation. Avant d'ingérer chaque fichier, le service vérifie que `usedBytes + fileSize ≤ quotaBytes`. Si le quota serait dépassé, le fichier est ignoré (log warning) et la sync continue sur les autres fichiers. Il sera re-tenté à la prochaine exécution si de l'espace a été libéré.

Voir **Annexe O — Politique de quotas** pour le détail complet du mécanisme d'enforcement.
