# ADR-006 — Stockage médias sur NAS local (pas S3/cloud)
...
Deux drivers séparés existent dès maintenant : `photoDriver()` pour la photothèque et `gedDriver()` pour la GED (Phase 7), permettant de connecter deux NAS différents si besoin.
...
- **Contrainte :** Pladigit n'est pas la source de vérité — le NAS peut être modifié en dehors de l'application. La synchronisation bidirectionnelle (tâche planifiée + drag-and-drop) atténue ce risque.
- **Contrainte :** performances dépendantes du réseau local ; le streaming Range HTTP (ADR-009) compense pour les vidéos.
- Pas de CDN possible sans couche intermédiaire.
