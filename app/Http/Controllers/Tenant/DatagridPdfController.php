...
class DatagridAdminController extends Controller
{
    ...
    {
        $columnNames = $table->columns->pluck('name')->toArray();
        ...
        $sortCol = ($data['default_sort_column'] ?? '') !== ''
            ? $data['default_sort_column']
            : null;

        // Vérifier que la colonne appartient bien à la grille (si renseignée)
        if ($sortCol !== null && ! in_array($sortCol, $columnNames, true)) {
            return response()->json([
                'errors' => ['default_sort_column' => ['Colonne inconnue dans cette grille.']],
            ], 422);
        }
