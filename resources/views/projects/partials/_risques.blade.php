...
$cellCriticality = function(string $prob, string $impact): string {
...
<details style="margin-top:4px;margin-bottom:4px;">
    <summary style="cursor:pointer;font-size:12px;color:var(--pd-muted);padding:6px 0;user-select:none;">
        ▸ Détail — {{ $active->count() }} risque{{ $active->count()>1?'s':'' }} actif{{ $active->count()>1?'s':'' }}
    </summary>
        ...
        <table class="pd-table">
            <thead>
                <tr>
                    ...
                    <th>Statut</th>
                    <th>Responsable</th>
                    @if($canManage)<th style="width:90px;"></th>@endif
                </tr>
            </thead>
            <tbody>
            @foreach($active->sortByDesc(fn($r)=>$r->score()) as $risk)
            @php $crit = $risk->criticality(); $cc = $critColors[$crit]; @endphp
            <tr>
                <td>
                    <div style="font-weight:500;margin-bottom:2px;">{{ $risk->title }}</div>
                    @if($risk->mitigation_plan)
                    <div style="font-size:11px;color:var(--pd-muted);">↳ {{ Str::limit($risk->mitigation_plan,70) }}</div>
                    @endif
                </td>
                <td style="color:var(--pd-muted);">{{ $catLabels[$risk->category] }}</td>
                <td><span class="pd-badge" style="background:{{ $cc['bg'] }};color:{{ $cc['text'] }};">{{ ucfirst($crit) }} ({{ $risk->score() }})</span></td>
                <td><span class="pd-badge" style="background:var(--pd-bg2);color:var(--pd-muted);">{{ $statLabels[$risk->status] }}</span></td>
                <td style="color:var(--pd-muted);">{{ $risk->owner?->name ?? '—' }}</td>
                @if($canManage)
                <td>
                    <div style="display:flex;gap:4px;">
                        <button type="button" class="btn-sm" style="padding:3px 8px;font-size:10px;"
                            onclick="openEditRisk({
                                action: '{{ route('projects.risks.update', [$project, $risk]) }}',
                                title: {{ Js::from($risk->title) }},
                                description: {{ Js::from($risk->description ?? '') }},
                                category: '{{ $risk->category }}',
                                probability: '{{ $risk->probability }}',
                                impact: '{{ $risk->impact }}',
                                status: '{{ $risk->status }}',
                                mitigation_plan: {{ Js::from($risk->mitigation_plan ?? '') }},
                                owner_id: '{{ $risk->owner_id ?? '' }}'
                            })">✎ Éditer</button>
                        <form method="POST" action="{{ route('projects.risks.destroy', [$project, $risk]) }}" onsubmit="return confirm('Supprimer ?')">
                            @csrf @method('DELETE')
...
<div id="modal-risk-edit" class="pd-modal-overlay">
    <div class="pd-modal pd-modal-md">
        ...
        <div class="pd-modal-body">
            <form id="form-risk-edit" method="POST">
                @csrf @method('PATCH')
                <div class="pd-form-group">
                    <label class="pd-label pd-label-req">Titre du risque</label>
                    <input type="text" id="re-title" name="title" class="pd-input" required>
                </div>
                <div class="pd-form-group">
                    <label class="pd-label">Description</label>
                    <textarea id="re-description" name="description" class="pd-input" rows="2"></textarea>
                </div>
                <div class="pd-form-row-3">
                    <div class="pd-form-group">
                        <label class="pd-label pd-label-req">Catégorie</label>
                        <select id="re-category" name="category" class="pd-input" required>
                            @foreach($catLabels as $val => $lbl)<option value="{{ $val }}">{{ $lbl }}</option>@endforeach
                        </select>
                    </div>
                    <div class="pd-form-group">
                        <label class="pd-label pd-label-req">Probabilité</label>
                        <select id="re-probability" name="probability" class="pd-input" required>
                            <option value="low">Faible</option>
                    ...
                    <div class="pd-form-group">
                        <label class="pd-label pd-label-req">Impact</label>
                        <select id="re-impact" name="impact" class="pd-input" required>
                            <option value="low">Faible</option>
                ...
                <div class="pd-form-group">
                    ...
                    <select id="re-status" name="status" class="pd-input" required>
                        @foreach($statLabels as $val => $lbl)<option value="{{ $val }}">{{ $lbl }}</option>@endforeach
                    </select>
                </div>
                <div class="pd-form-group">
                    <label class="pd-label">Plan de mitigation</label>
                    <textarea id="re-mitigation" name="mitigation_plan" class="pd-input" rows="2"></textarea>
                </div>
                <div class="pd-form-group">
                    <label class="pd-label">Responsable du suivi</label>
                    <select id="re-owner" name="owner_id" class="pd-input">
                        <option value="">—</option>
        ...
        <div class="pd-modal-footer">
            <button type="button" class="pd-btn pd-btn-secondary" onclick="document.getElementById('modal-risk-edit').classList.remove('open')">Annuler</button>
            <button type="submit" form="form-risk-edit" class="pd-btn pd-btn-primary">Enregistrer</button>
        </div>
    </div>
</div>

<script>
window.openEditRisk = function(data) {
    var form = document.getElementById('form-risk-edit');
    form.action = data.action;
    document.getElementById('re-title').value       = data.title;
    document.getElementById('re-description').value = data.description;
    document.getElementById('re-category').value    = data.category;
    document.getElementById('re-probability').value = data.probability;
    document.getElementById('re-impact').value      = data.impact;
    document.getElementById('re-status').value      = data.status;
    document.getElementById('re-mitigation').value  = data.mitigation_plan;
    document.getElementById('re-owner').value       = data.owner_id;
    document.getElementById('modal-risk-edit').classList.add('open');
};
</script>
@endif
