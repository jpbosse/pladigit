{{-- _stakeholders.blade.php — Carte d'adhésion au changement (matrice 2D) --}}
@php
...
$inflOrder = ['high','medium','low']; // clés DB
...
{{-- ── MATRICE 2D influence × adhésion ── --}}
<div class="pd-card" style="overflow-x:auto;margin-bottom:14px;padding:0;">
    <table style="width:100%;border-collapse:collapse;table-layout:fixed;">
        <thead>
            <tr>
                <th style="width:72px;padding:10px 8px;border-bottom:1px solid var(--pd-border);border-right:1px solid var(--pd-border);font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:.04em;color:var(--pd-muted);text-align:center;background:var(--pd-bg2);">
                    Influence<br><span style="opacity:.6;">↕</span> / Adhésion <span style="opacity:.6;">→</span>
                </th>
                @foreach($adhOrder as $adh)
                ...
                <th style="padding:10px 8px;border-bottom:1px solid var(--pd-border);{{ !$loop->last ? 'border-right:1px solid var(--pd-border);' : '' }}text-align:center;">
                    <span style="display:inline-block;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:700;background:{{ $cfg['bg'] }};color:{{ $cfg['text'] }};">
                        {{ $cfg['label'] }}
                    </span>
                    @if($count)
                    <div style="font-size:9px;color:var(--pd-muted);margin-top:2px;">{{ $count }}</div>
                    @endif
                </th>
                @endforeach
            </tr>
        ...
        <tbody>
            ...
            <tr>
                <td style="padding:6px 8px;border-right:1px solid var(--pd-border);{{ !$loop->last ? 'border-bottom:1px solid var(--pd-border);' : '' }}text-align:center;background:var(--pd-bg2);vertical-align:middle;">
                    ...
                    @if($infl==='high')<div style="font-size:9px;color:var(--pd-muted);margin-top:1px;">décisif</div>@endif
                    @if($infl==='low')<div style="font-size:9px;color:var(--pd-muted);margin-top:1px;">limité</div>@endif
                </td>
                @foreach($adhOrder as $adh)
                @php
                ...
                @endphp
                <td style="padding:10px 6px;vertical-align:top;min-height:56px;{{ !$loop->last ? 'border-right:1px solid var(--pd-border);' : '' }}{{ !$loop->parent->last ? 'border-bottom:1px solid var(--pd-border);' : '' }}background:{{ $cellBg }};"
                    x-data="{}">
                    @if($items->isEmpty())
                        @if($warning)
                        <div style="text-align:center;padding:10px 0;opacity:.18;font-size:18px;">⚠</div>
                        @elseif($target)
                        <div style="text-align:center;padding:10px 0;opacity:.12;font-size:18px;">★</div>
                        @else
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:6px;justify-content:center;">
                        ...
                        <div style="position:relative;" x-data="{ open: false }">
                            <div class="sh-avatar"
                                 ...
                                 :style="open ? 'transform:scale(1.18);z-index:10' : ''">
                                {{ $sh->initials() }}
                            </div>
                            <div x-show="open" x-cloak
                                 style="position:absolute;bottom:calc(100% + 8px);left:50%;transform:translateX(-50%);white-space:nowrap;background:var(--pd-surface);border:1px solid var(--pd-border);border-radius:8px;box-shadow:0 6px 20px rgba(0,0,0,.13);padding:9px 13px;z-index:200;min-width:160px;">
                                ...
                                @if($sh->notes)
                                <div style="font-size:10px;color:var(--pd-muted);margin-top:6px;border-top:1px solid var(--pd-border);padding-top:5px;white-space:normal;max-width:200px;">{{ $sh->notes }}</div>
                                @endif
    ...
    <div style="display:flex;gap:18px;padding:9px 14px;border-top:1px solid var(--pd-border);background:var(--pd-bg2);flex-wrap:wrap;">
        <div style="display:flex;align-items:center;gap:5px;font-size:10px;color:var(--pd-muted);">
            <span style="width:10px;height:10px;background:#FFF5F5;border:1px solid #FECACA;border-radius:2px;display:inline-block;"></span>
            Zone critique — haute influence, faible adhésion
        </div>
        <div style="display:flex;align-items:center;gap:5px;font-size:10px;color:var(--pd-muted);">
            <span style="width:10px;height:10px;background:#F0FDF4;border:1px solid #86EFAC;border-radius:2px;display:inline-block;"></span>
            Zone cible — alliés à mobiliser en priorité
        </div>
    </div>
</div>

{{-- ── Tableau détail repliable ── --}}
<details>
    ...
    <div class="pd-card" style="margin-top:8px;">
        ...
        <table class="pd-table">
            <thead><tr><th>Nom</th><th>Rôle</th><th>Adhésion</th><th>Influence</th><th>Notes</th>@if($canManage)<th></th>@endif</tr></thead>
            <tbody>
...
<div id="modal-sh" class="pd-modal-overlay">
    <div class="pd-modal pd-modal-md">
        <div class="pd-modal-header">
            <div>
                <div class="pd-modal-title">Nouvelle partie prenante</div>
                <div class="pd-modal-subtitle">Positionnée automatiquement sur la carte</div>
            </div>
        ...
        <div class="pd-modal-body">
            <form id="form-sh" method="POST" action="{{ route('projects.stakeholders.store', $project) }}">
                ...
                <div class="pd-form-group">
                    ...
                    <input type="text" name="role" class="pd-input" placeholder="Ex: Commanditaire, Utilisateur clé" required>
                </div>
                <div class="pd-form-row-2">
                    <div class="pd-form-group">
                        <label class="pd-label pd-label-req">Adhésion</label>
                        <select name="adhesion" class="pd-input" required>
                            @foreach($adhesionCfg as $val => $cfg)
                    ...
                    <div class="pd-form-group">
                        ...
                        <select name="influence" class="pd-input" required>
                            @foreach($influenceLbls as $val => $lbl)
